...
def create_encoders(preset_name, dict_size):
    if preset_name == 'rnn':
        ...
        # create config dictionaries with all the parameters for your encoders
        char_config = {'embed':{'num_chars': dict_size, 'embedding_dim': 300, 
                                'sparse': False, 'padding_idx': 0
                                }, 
                        'rnn':{'input_size': [300,2048], 
                               'hidden_size': [1024, 300], 
                               'n_layers': [1, 1], 'batch_first': True, 
                               'bidirectional': True, 'dropout': 0, 
                               'max_len': 128
                               }, 
                        'att':{'in_size': 600, 'hidden_size': 128, 'heads': 1
                               },
                        'VQ':{'n_layers': 0, 'n_embs': [], 'emb_dim': []},
                        'app_order': [0, 0]
                        }
        # calculate the required output size of the image encoder
        out_size = char_config['rnn']['hidden_size'][-1] * 2 ** \

...
def iterate_audio_flickr(f_nodes, batchsize, visual, audio, frames = 1024, shuffle = True, test = False):
    if shuffle:
        # optionally shuffle the input
        np.random.shuffle(f_nodes)
    if not test:
...
def iter_text_flickr(f_nodes, batchsize, visual, text, chars = 200, shuffle=True, test = False):
    if shuffle:
        # optionally shuffle the input
        np.random.shuffle(f_nodes)
    if not test:
    ...
    for i in range(0,x):
        for start_idx in range(0, len(f_nodes) - batchsize + 1, batchsize):
            ...
            for ex in excerpt:
                ...
                caption.append('^', cap.lower())
            # converts the sentence to character ids. 
            caption = char_2_index(caption.lower(), batchsize, chars)
            images_shape = np.shape(images)
            # images should be shape (batch_size, 1024). images_shape[1] is collapsed as the original features are of shape (1,1024) 
            images = np.float64(np.reshape(images,(images_shape[0],images_shape[2])))

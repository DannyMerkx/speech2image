...
# training routine 
def train_epoch(epoch, img_net, cap_net, optimizer, f_nodes, args.dict_loc, batch_size):
    global iteration
    img_net.train()
    cap_net.train()
    ...
    train_loss = 0
    num_batches =0
    for batch in batcher(f_nodes, batch_size, args.visual, args.cap, max_words = 60, shuffle = True):
...
def recall(data, at_n, c2i, i2c, prepend):
    ...
    if c2i:
        ...
        iterator = batcher(data, args.batch_size, args.visual, args.cap, args.dict_loc, max_words = 60, shuffle = False)

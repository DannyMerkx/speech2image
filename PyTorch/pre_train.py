...
# -*- coding: utf-8 -*-
"""
Created on Tue Apr  3 14:12:50 2018
...
img_optimizer = torch.optim.SGD(img_net.parameters(), lr= 0.00005, momentum=0.9)
audio_optimizer = torch.optim.SGD(img_net.parameters(), lr= 0.00005, momentum=0.9)


def pre_training(img_net, audio_net, img_optimizer, audio_optimizer, f_nodes, batch_size):
    img_net.train()
    pre_train_loss = 0
    num_batches = 0
    for batch in batcher(f_nodes, batch_size, shuffle = True):
        ...
        # calculate the loss
        img_loss = batch_hinge_loss(img_embedding, audio_embedding, args.loss)
        audio_loss = batch_hinge_loss(audio_embedding, audio_embedding, args.loss)
        ...
        # add loss to average
        pre_train_loss += img_loss.data
        
    return pre_train_loss/num_batches
# training routine 
...
for x in range(3):
    pre_train_loss = pre_training(img_net, audio_net, img_optimizer, audio_optimizer, f_nodes, batch_size )
